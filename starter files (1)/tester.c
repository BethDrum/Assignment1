...
void implementROI(int a, int b, int c, int d);
...
int main()
{
    ...
    //  3 = madrid
    //  4 = paris
    //inbound start, in end, outbound start, out end
    implementROI(1, 3, 3, 1);
}
...
//ROI function
void implementROI(int enteredStOut, int destinationOut, int enteredStIn, int destinationIn){
    //check incoming parameters
    if (enteredStOut < 0 || enteredStOut > 4 || destinationOut < 0 || destinationOut > 4 || enteredStIn < 0 || enteredStIn > 4 || destinationIn < 0 || destinationIn > 4){
    ...
    if (result1 == SUCCESS && result2 == SUCCESS){
        ...
        if (dijkstraTable1.table[destinationOut].distance < dijkstraTable2.table[destinationIn].distance){
            ...
            printf("The company will embark on the inbound trip, as the distance of the inbound trip is: %d minutes less", dijkstraTable1.table[destinationOut].distance - dijkstraTable2.table[destinationIn].distance);
        }else{
            printf("The company will embark on both as they take the same amount of time, %d minutes.\n", dijkstraTable1.table[destinationOut].distance);
        }
    }else{
        //print error message if either dijskra failed
